...
- 一覧を崩さずに詳細を見せる
- URL共有できる（`?p=slug` で直接モーダルを開ける）
- 戻るボタンで自然に閉じられる（クリックで開いた場合）

### 7-2. 開く/閉じるの履歴挙動（クリック vs 直リンク）

- **クリックで開いた場合**
  - 開く：`?p=slug` を `pushState`
  - 閉じる：`history.back()`（戻るで閉じたのと同じ体験にする）
- **直リンク（最初から ?p が付いている等）の場合**
  - 閉じる：`?p` を `replaceState` 相当で消す（履歴を壊しにくい）
...
- モーダル表示中は `body` を `position: fixed` にして背景スクロールを止める
- 閉じるときに元のスクロール位置へ復帰

### 8-2. キーボード操作

- `Escape`：閉じる
- `ArrowLeft / ArrowRight`：画像スライド

### 8-3. 外側クリックで閉じる

- オーバーレイ背景をクリックした場合のみ close（中身クリックは閉じない）

---

## 9. 画像ギャラリー（表示・操作）

- `thumb` + `images[]` を結合して `gallery` を作成
- 矢印の **丸ボタン**で前後移動
- さらに **タッチ（pointerType: touch）** のスワイプで前後移動
  - 縦スクロールと競合しないよう、横移動が優勢なときだけスワイプ扱いにする

---

## 10. テーマ（Light / Dark / Auto）

- `ThemeControls.tsx` が `<select>` を提供
- `theme.ts` が `localStorage('theme-mode')` に保存しつつ、`body` に class を付与
  - Dark のとき：`body.classList.add('theme-dark')`
  - Auto（system）のとき：`matchMedia('(prefers-color-scheme: dark)')` の change を監視して追随
